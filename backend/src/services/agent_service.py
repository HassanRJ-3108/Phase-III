...
This service initializes and manages the Gemini Agent (via OpenAI Agents SDK)
for processing user messages and orchestrating MCP tool calls.
"""
...
class AgentService:
    """Service for managing Gemini agent interactions with MCP tools.

    This service provides natural language interface to task management
    by orchestrating Gemini models (via OpenAI-compatible API) with MCP server tools.
    """

    def __init__(self, api_key: str, model: str = "gemini-2.0-flash-exp", base_url: str = "https://generativelanguage.googleapis.com/v1beta/openai/", mcp_base_url: str = "http://localhost:8001"):
        """Initialize the agent service.

        Args:
            api_key: Gemini API key
            model: Gemini model name (default: gemini-2.0-flash-exp)
            base_url: Gemini API base URL (OpenAI-compatible endpoint)
            mcp_base_url: Base URL for MCP server
        """
        logger.debug(f"Initializing AgentService with model={model}, base_url={base_url}, mcp_base_url={mcp_base_url}")
        ...
        # Create AsyncOpenAI client for Gemini's OpenAI-compatible endpoint
        self.client = AsyncOpenAI(api_key=api_key, base_url=base_url)
        
        self.system_prompt = self._build_system_prompt()
        self.tools = self._register_mcp_tools()

        logger.info(f"AgentService initialized with {len(self.tools)} tools using Gemini via AsyncOpenAI")
...
"""
    ...
    def _register_mcp_tools(self) -> list[dict[str, Any]]:
        ...
        return [
             ...
             },
            {
                ...
                "function": {
                    "name": "complete_task",
                    "description": "Mark a task as completed or incomplete. Use this when user says 'mark as done', 'complete this', 'mark as completed', 'finish task', etc. You MUST have the task_id first - if you don't know it, call list_tasks first to find it.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id": {
                                "type": "string",
                                "description": "UUID of the task to mark as complete/incomplete"
                            },
                            "completed": {
                                "type": "boolean",
            ...
            },
            {
                ...
                "function": {
                    "name": "update_task",
                    "description": "Update an existing task's title or description. Use this when user wants to edit, modify, rename, or change task details. You MUST have the task_id first - if you don't know it, call list_tasks first to find it.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id": {
    ...
    async def process_user_message(
        ...
        logger.info(f"Tools count: {len(self.tools)}")
...
async def get_agent_service() -> AgentService:
    ...
    Returns:
        Configured AgentService instance using Gemini API
    """
    logger.info("=== get_agent_service called ===")
    logger.info(f"GEMINI_API_KEY loaded: {'YES' if settings.gemini_api_key else 'NO - MISSING!'}")
    logger.info(f"GEMINI_MODEL: {settings.gemini_model}")
    logger.info(f"GEMINI_BASE_URL: {settings.gemini_base_url}")
    ...
    try:
        ...
        logger.error(f"FAILED to create AgentService: {type(e).__name__}: {str(e)}")
        raise
